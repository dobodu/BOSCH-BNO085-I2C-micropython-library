...
from bno08x_debug import channels, reports
...
BNO_REPORT_MAGNETOMETER = const(0x03)			# Magnetic field calibrated (in µTesla). The fully calibrated magnetic field measurement.
BNO_REPORT_LINEAR_ACCELERATION = const(0x04)	# Linear acceleration (m/s2). Acceleration of the device with gravity removed
...
class BNO08X:  # pylint: disable=too-many-instance-attributes, too-many-public-methods
    ...
    def __init__(self, reset=None, debug=False):
        self._debug = debug
        self._reset = reset
        self._dbg("BNO08X : INITIALISATION...")
    ...
    def hard_reset(self):
        ...
        self._reset = Pin(BNO_RESET_PIN, Pin.OUT)
        self._reset.value(1)
        time.sleep(0.01)
        self._reset.value(0)
        time.sleep(0.01)
